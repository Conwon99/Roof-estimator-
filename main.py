...
# Slope factors: sqrt(run² + rise²) / run for a standard gable roof
PITCH_FACTORS: dict[str, float] = {
    "flat":             1.000,  # 0:12
    "low_2_12":         1.014,  # 2:12
    "standard_4_12":    1.054,  # 4:12 — most common residential
    "moderate_6_12":    1.118,  # 6:12
    "steep_9_12":       1.250,  # 9:12
    "very_steep_12_12": 1.414,  # 12:12
}
...
def polygon_area_sqft(coords: list[tuple[float, float]]) -> float:
    """Shoelace formula on (lon, lat) pairs, projected to metres then converted to ft²."""
    if len(coords) < 3:
        return 0.0
    R = 6_371_000
    ref_lat = coords[0][1]
...
def _extract_best_way(
    elements: list[dict],
    lat: float,
    lon: float,
) -> Optional[dict]:
    """From a list of OSM elements, return the best building footprint dict."""
    ...
    for way in ways:
        ...
        candidates.append((way, coords, area))

    if not candidates:
        return None

    # Prefer the footprint that contains the geocoded point; else take closest centroid
    containing = [c for c in candidates if point_in_polygon(lon, lat, c[1])]
    if containing:
        best_way, _, best_area = max(containing, key=lambda c: c[2])
    else:
        best_way, _, best_area = min(candidates, key=lambda c: centroid_dist_sq(c[1], lon, lat))

    return {"area_sqft": best_area, "osm_id": best_way["id"], "tags": best_way.get("tags", {})}
...
async def geocode_address(address: str) -> dict:
    """Geocode with Nominatim. Returns lat, lon, display_name, osm_type, osm_id."""
...
# ── Main building footprint fetcher ──────────────────────────────────────────
...
async def fetch_building_footprint(
    ...
    """
    Strategy:
    1. If Nominatim gave us a way → fetch that way directly.
    2. If Nominatim gave us a relation → fetch the relation's outer ring.
    3. Fallback: bbox search + point-in-polygon / closest-centroid selection.
    """
    async with httpx.AsyncClient() as client:
...
async def estimate_roof(req: EstimateRequest):
    ...
    if building:
        ...
        tags  = building["tags"]
        floors = int(tags.get("building:levels", 1) or 1)
        ...
        return {
            ...
            "message": (
                ...
                "The address was geocoded — use the satellite view link to measure manually."
            ),
        }
